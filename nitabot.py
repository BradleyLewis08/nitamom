...
class ZombieWorldState:
    ...
    def __init__(self, fr, fl, br, bl, arm1, arm2, arm3, arm4):
        ...
        self.gettingBerry = False
        self.isRight = True #unused
...
def main():
    ...
    i=0
    j=10 #j starts counting down when it has stopped moving and cycles until it starts moving again
    arm_pos = 1
    stopped = 0 #unused
    ...
    while(robot_not_dead == 1):
     ...
     #------------------CHANGE CODE BELOW HERE ONLY--------------------------   #called every timestep
        ...
        if i==0: #if just starting out, initialize prev_pos
            prev_pos = [0, 0, 0]

        curr_pos = gps.getValues()
        distance_traveled = math.sqrt((curr_pos[0] - prev_pos[0])**2 + (curr_pos[1] - prev_pos[1])**2 + (curr_pos[2] - prev_pos[2])**2)
        prev_pos = curr_pos

        if distance_traveled < .0005 or j < 10:
            if j == 10:
                if arm_pos == 1:
                    zombieState.arm1.setPosition(-1)
                    j -= 1
                else:
                    zombieState.arm1.setPosition(1)
                    j -= 1
                ...
                j -= 1
                zombieState.base_turn_right() #if stopped, start turning right
