...
class ZombieWorldState:
    ...
    def detect_obstacles(self, prev_location, curr_location, front_image, left_image, right_image):
        ...
        self.nearWall = self.frontWallPixels[0] > .2*CAMERA_SIZE
...
def main():
    ...
    while(robot_not_dead == 1):
     ...
     #------------------CHANGE CODE BELOW HERE ONLY--------------------------   #called every timestep
        ...
        if grinding_turn_counter > 0:
            zombieState.base_backwards()
            grinding_turn_counter -= 1
            if grinding_turn_counter == 0:
                zombieState.isTrapped = False
                zombieState.isGrinding = False
                emergency_turn_counter = 0
        ...
        if i % 15 == 0:
            zombieState.detect_obstacles(prev_location, curr_location, cameraData[0], cameraData[1], cameraData[2])
            prev_location = curr_location

        if zombieState.isTrapped:
            emergency_turn_counter = 15
        # elif zombieState.isGrinding:
        #    grinding_turn_counter = 2
        #    print("grinding.........")
            ...
            if not i%25:
                if desperate:
                    ...
                    # print("i am here")
                    choice = random.choice([[10, 7, 10, 7], [7, 10, 7, 10], [10, 10, 10, 10]])
                    zombieState.custom_turn(choice[0], choice[1], choice[2], choice[3])
